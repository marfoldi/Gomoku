...
// Szukseges, hogy az toString fuggvenyt hasznalhassuk Maybe Player tipusu ertekekre.
instance toString (Maybe Player) where
	toString (Just X) = "A jatekot nyerte: X"
	toString (Just O) = "A jatekot nyerte: O"
	toString Nothing = "Nincs nyertes."
...
checkNInARow length numbers = getNext 1 (sort numbers) == length 
	where
	getNext :: Int [Int] -> Int
	getNext _ [] = 0
	getNext actualLength [x] = actualLength
	getNext actualLength [x0:x1:xs]
		| actualLength == length = length
		| x0+1 == x1 = getNext (actualLength+1) [x1:xs]
		| x0 == x1 = getNext actualLength [x1:xs]
		| otherwise = getNext 1 [x1:xs]
...
checkBoardForN length (player, (x,y)) board direction
	| checkNInARow length (map (project direction) (filter (lineFilter (player, (x,y)) direction) board ++ [(player, (x, y))])) = Just player
	| otherwise = Nothing
...
playGame length positions = doStep positions X []
	where
	doStep [] _ _ = Nothing
	doStep [x:xs] player board
		| checkBoardForN length (player, x) board Horizontal <> Nothing || checkBoardForN length (player, x) board Vertical <> Nothing || checkBoardForN length (player, x) board LeftDiagonal <> Nothing || checkBoardForN length (player, x) board RightDiagonal <> Nothing = Just player
		| otherwise = doStep xs (nextPlayer player) (board ++ [(player, x)])
...
startGame length positions
	| areValidSteps positions = toString (playGame length positions)
	| otherwise = "Hibas lepessorozat."
